...
| 输入 | 模式 | 处理方式 |
|---|---|---|
| `/sql setup` | 交互式配置 | 逐一询问 6 个问题，然后执行配置 |
...
如果 Q1 选择了 oracledb，Q2 改为询问：
> 请提供 Oracle 连接信息，格式如下：
> ```
> 用户名  密码  主机:端口/服务名
> ```
> 例如: `xh_com  JEFFv#22com  d88.lis-china.com:1521/lisdb`
...
> 是否需要为 `/sqltest` 自动化测试配置 API 基地址？（y/n）
> 当前默认: `http://localhost:5000`
> 如果选 `n`，保持默认值。

等待用户回答：

- **`n` / `否` / 回车** → 保持默认 `http://localhost:5000`，不修改。
- **`y` / `是`** → 继续询问：

> 请输入 API 基地址：
> 例如: `http://localhost:5000` 或 `https://127.0.0.1:18527`

记下用户提供的地址。去掉末尾的 `/`（如果有）。

如果用户回复 `取消` 或 `退出`，保持默认值。

**存储：** 将以下标记写入 `.claude/skills/sqltest/SKILL.md` 顶部：

```
<!-- api-base-url: <用户提供的地址> -->
```

如果该标记已存在，更新它；如果不存在，插入新行。

---

### Q6: 自动化测试 Token 获取

> 是否需要为 `/sqltest` 自动化测试配置 Token 获取方式？（y/n）
> 如果选 `n`，跳过此步骤。
...
> 请提供 Token 获取接口的完整 curl 命令（包含 URL 和参数）。
> 返回的 JSON 中必须包含 `data.AccessToken` 字段。
> 例如:
> ```
> curl -sk -X POST "https://d40.lis-china.com:18801/api/Account/GetIssueTokenInfo?userNo=LIS3&tokenGuid=123123&moduleId=H127&callModuleId=H127"
> ```

记下用户提供的 curl 命令中的 URL 和方法。如果用户提供的是 curl 命令，解析出：
- HTTP 方法（默认 POST）
- 完整 URL（含 query string）

如果用户回复 `取消` 或 `退出`，跳过 token 配置，继续完成提示。

**存储：** 将以下标记写入 `.claude/skills/sqltest/SKILL.md` 顶部：

```
<!-- token-url: <METHOD> <URL> -->
```
...
**Oracle（oracledb）：** 使用上表 "Oracle 配置示例" 的格式，env 字段为 `ORACLE_USERNAME`、`ORACLE_PASSWORD`、`ORACLE_CONNECTION_STRING`。

**SQLite：** `{"SQLITE_FILEPATH": "<路径>"}`

**各数据库类型的环境变量映射：**

| 类型 | `--prebuilt` 值 | env 格式 | 端口默认值 |
|---|---|---|---|---|
| postgres | postgres | `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DATABASE` | 5432 |
| mysql | mysql | `MYSQL_HOST`, `MYSQL_PORT`, `MYSQL_USER`, `MYSQL_PASSWORD`, `MYSQL_DATABASE` | 3306 |
| mssql | mssql | `MSSQL_HOST`, `MSSQL_PORT`, `MSSQL_USER`, `MSSQL_PASSWORD`, `MSSQL_DATABASE` | 1433 |
| oracledb | oracledb | `ORACLE_USERNAME`, `ORACLE_PASSWORD`, `ORACLE_CONNECTION_STRING` | (见连接串) |
| sqlite | sqlite | `SQLITE_FILEPATH` | — |

**Oracle 配置示例：**

```json
{
  "toolbox-oracle-lisdb": {
    "command": "npx",
    "args": ["-y", "@toolbox-sdk/server", "--prebuilt=oracledb", "--stdio"],
    "env": {
      "ORACLE_USERNAME": "xh_com",
      "ORACLE_PASSWORD": "JEFFv#22com",
      "ORACLE_CONNECTION_STRING": "d88.lis-china.com:1521/lisdb"
    }
  }
}
```
